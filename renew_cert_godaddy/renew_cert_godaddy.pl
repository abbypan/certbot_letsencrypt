...
sub create_dns_rr {
  ...
  my $data = [ { data => $acme_rr, name => $ACME_SUBDOM, ttl => 3600, type => "TXT" } ];
...
sub curl_godaddy_api {
  ...
  my $cmd =
    qq[curl -H "Accept: application/json" -H "Content-Type: application/json" -H "Authorization: sso-key $access_key_id:$access_key_secret" -X $method "$api" -d '$json_data' ];
  printf $cmd, "\n";
  system( $cmd);
}
